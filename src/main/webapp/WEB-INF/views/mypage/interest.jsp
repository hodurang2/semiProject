...
<script>
  ...
  // 선택한 관심지역 시/도, 시군구 확인
  const fnBeforeInterest = () => {
    $('#interestSido').append('<option value="${sessionScope.user.interestSido}">${sessionScope.user.interestSido}</option>');
    $('#interestSigungu').append('<option value="${sessionScope.user.interestSigungu}">${sessionScope.user.interestSigungu}</option>');
  }
  
  // 시도 셀렉트박스 클릭 시 기존 관심지역 시/도 삭제 후 초기화
  const fnRemoveInterest = () => {
    $('#interestSido').one('click', () => {
      $("#interestSido").empty();
      fnResetInterest();
    })
  }
